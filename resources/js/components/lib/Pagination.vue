<template>
  <nav v-if="pages() > 1" aria-label="Page navigation">
    ...
    >
      ...
      <li
        v-for="(page,index) in pagesWithDot"
        :class="{active: page === current}"
        :key="index"
        class="page-item"
      >
        <input
          v-if="activeIndex === index && !(typeof page === 'number')"
          v-model="inputValue"
        ...
        <a
          ...
          style="font-size: 20px;
            font-weight: 900;"
          :class="{disabled:(page === current) || !(typeof page === 'number')}"
      ...
      <li class="page-item" :class="{disabled: current >= lastPage}">
        <a
          ...
          @click.prevent="nextBtnClick(current)"
          href="#"
          aria-label="Next"
        >
